...
export default {
  ...
  methods: {
    ...
    sendMemo() {
        const baseUrl = "https://asia-northeast1-writedate.cloudfunctions.net/widgets/writeDate";
        console.log("SEND_MEMO");
        this.$http.post(`${baseUrl}`,{msg : this.message})
        .then((result) => {
          console.log("CALL FIREBASE RESULT: "+result);
          this.message=""
          // this.getMyNumber()
    ...
    getMyNumber(){
      const baseUrl = "https://asia-northeast1-writedate.cloudfunctions.net/widgets/ReadUserDate";
      var temp;
      this.$http.get(`${baseUrl}`)
        .then((result) => {
          document.getElementById("memory").style.display = "block"
          console.log("SEND_MEMO FIREBASE RESULT_NUMBER: "+JSON.stringify(result));
          const responseArray = result.data.number;
        
          const arr = responseArray.match(/\((.*?)\)/g);
          console.log(arr)
          ...
          for(var i=0; i<arr.length; i++) {
            ...
            this.array[i][0]=temp[0];
            this.array[i][1]=temp[1];
          }
